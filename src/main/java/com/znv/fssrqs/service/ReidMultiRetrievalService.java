...
public class ReidMultiRetrievalService {
    ...
    private JSONObject getTemplateParams(JSONObject requestParams) {
        ...
        if (features.size() > 0) {
            String url = String.format(CommonConstant.ThousandSights.GET_FEATURE_URL,systemDeviceLoadTask.getReidUnit().getIP(), systemDeviceLoadTask.getReidUnit().getPort());
